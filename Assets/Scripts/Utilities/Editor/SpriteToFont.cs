...
namespace GenericUtility
{
    public class SpriteToFont : MonoBehaviour
    {
        ...
        {
            Debug.LogWarning("Create My Font Sprite");

            if (Selection.objects == null) { return; }

            if (Selection.objects.Length == 0) { Debug.LogWarning("没有选中 Sprite 文件，需要将 Sprite Mode 设置成 Multiple，切分好，并且以名字的最后一个字符当做 ASCII 码"); return; }

            Object tempObj = Selection.objects[0];

            if (tempObj.GetType() != typeof(Texture2D)) { Debug.LogWarning("选中的并不是图片文件"); return; }

            string selectionPath = AssetDatabase.GetAssetPath(tempObj);
            ...
            if (selectionPath.Contains(resoursePath))
            {
                ...
                if (sprites.Length > 0)
                {
                    ...
                    Texture2D tex = tempObj as Texture2D;
                    //创建字体材质，并且将图片设置好  
