Shader "Hidden/SC Post Effects/Caustics"
{
	...
	float4 Frag(v2f i) : SV_Target
	{
		...
		//return float4(frac(worldPos.xyz), 1.0);
		
		float2 uv = (worldPos.xz * _CausticsParams.x) ;

		float3 caustics1 = SAMPLE_TEX(_CausticsTex, sampler_CausticsTex, uv + (_Time.y * float2(_CausticsParams.y, _CausticsParams.z))).rgb;
		float3 caustics2 = SAMPLE_TEX(_CausticsTex, sampler_CausticsTex, (uv * 0.8) -(_Time.y * float2(_CausticsParams.y, _CausticsParams.z))).rgb;
